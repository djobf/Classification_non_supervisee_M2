...
Nous travaillons sur les données crabs. Les données contiennent 5 mesures de morphologies faitent sur un échantillions de 200 crabes (taille du lobe frontal, largeur arrière, longueur de la carapace, largeur de la carapace et prondondeur du corps). Les crabs sont divisées en 4 classes, réparties selon l'espèce (bleue ou orange) et le sexe. L'objectifs serait d'imaginer une étude de clustering qui ne les utiliserait pas et par la suite de comparer la classification obtenue avec celles en espèce et en sexe.
...
L'ensembles des variables semblent avoir une distribution normale. On ne peut pas comparer les 200 crabes graphiquement en fonction des 5 mesures de morphologies car il faudrait un graphique en 5 dimensions et l'oeil humain s'arrête à 3 dimensions. Une réduction de dimension s'impose donc, pour cela on va utiliser l'ACP. L'ACP permet de réduire le nombre de variables d'un ensemble de données en projetant ces variables dans un nouvel espace de dimensions inférieures. L'objectif est de projeter les données dans un nouvel espace où la première composante principale explique autant de variance que possible, de même pour la deuxième composante principale et ainsi de suite.

## 2. Etude de l'ACP

### 2.a. Proportion de variance expliqué par les deux premières composantes
...
La première composante principale explique 95,77% de la variance des données ainsi elles explique la majorité de la variance des données. En comparaisons la deuxieme composantes principale n'explique que 3% de la variance des données ce qui parrait minime.
...
Deux groupe semblent se dégager de la projection des données sur leur deuxième composante principale.
...
L'ACP ne semblent pas former 4 groupes distincs mais seulement 2 pour les données crabs, en effet ils semblerais qu'on ne garde pas assez d'information sur la répartion des données pour les séparer en 4 groupes. Voyons ce que cela donne en affichant le sexe et l'espèce
...
En effet l'ACP a seulement séparé les males et les femelles, cepandant les distinctions entre les espèces sont faites par leur couleur or l'ACP ne prend pas en compte cette données. On peut donc suposser que l'ACP reste efficasse dans le cadre des données continues.
...
La plupart des variables semblent collés à la distrubition de la loi normale. Regardons la répartion des données à travers l'ACP :
...
L'ACP semblent mettrent en évidence 3 groupes, un à gauche, un en bas et un à droite.
...
```
L'algorithme des k-means séparent en effet les données comme les 3 groupes que nous avions distingués. Regardons les coefficients de silhouette moyen pour chaque cluster :
...
```
Les coefficients de silhouette moyen sont relativement élevé pour chaque cluster, les points sont en moyenne beaucoup plus proche de leur groupe que du groupe voisin. Regardons la somme des carrés intra et inter-cluster:
```{r}
...
for (i in 1:3) {
  ...
  kmeans_result <- kmeans(variables_kmeans, centers = K, iter.max = i)
...
On constate que les cluster évolue au fil des itérations et que leur centres se déplace, en effet les nouveaux centres de chaque nouveau cluster sont calculés en prenant la moyenne des points attribués pour chacun d'entre eux.
...
```
La fonction Ward favorise la formation de clusters homogènes en minimisant la variance intra-cluster. Les classes choisies semblent similaire à celle choisie par la méthode des k-means.
...
Les coefficients de silhouette moyen sont légèrement inférieur à la méthode des k-means mais restent relativement élevé pour chaque cluster, les points sont en moyenne beaucoup plus proche de leur groupe que du groupe voisin.
...
```
La fonction single linkage semblent avoir crée une très grande classe et deux petites, en effet elle a tendance à former des clusters étendus.

```{r}
aggregate(silhouette(hierarchical_clusters, dist(variables_kmeans))[, "sil_width"], by=list(Cluster=hierarchical_clusters), FUN=mean)$x
```
Les coefficients de silhouette moyen sont très élever pour le deuxieme et troisième cluster, en revanche il est très proche de 0 pour le premier cluster, les points sont aussi proche de leur groupe que du groupe voisin pour le premier cluster.

```{r}
paste("Intra-cluster",sum(hierarchical_result$height[hierarchical_clusters]))
paste("Inter-cluster",sum(hierarchical_result$height)-sum(hierarchical_result$height[hierarchical_clusters]))
```
L'inertie inter-classe reste élevée par rapport à l'inertie intra-classe, les clusters sont bien séparés les uns des autres.
...
aggregate(silhouette(hierarchical_clusters, dist(variables_kmeans))[, "sil_width"], by=list(Cluster=hierarchical_clusters), FUN=mean)$x
```
Les coefficients de silhouette moyen sont relativement élevé pour chaque cluster, les points sont en moyenne beaucoup plus proche de leur groupe que du groupe voisin.
...
```
La fonctions group average donne des classes similaires aux méthode précédentes, on peut l'expliquer par le fait qu'elle est moins sujette à la formation de clusters étendus.
...
La majorité des méthodes donnent des résultat satisfaisants, à l'exeption de single linkage. On retrouve à chaque foit les 3 variétés de blé espérés. Ceci valide donc l'efficacité des différentes méthodes de classification.
